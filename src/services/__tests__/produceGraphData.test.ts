      ...
      expectedGraph: {
        Facebook: {
          monthlyWordsPerConversation: {
            "conversation-0": [{ year: 2023, month: 12, sentCount: 30, receivedCount: 15 }]
          },
          dailyWords: [
            {
              year: 2023,
    ...
    describe(name, () => {
      ...
      Object.entries(expectedGraph).forEach(([key, expectedData]) => {
        it(`should produce the expected graph data for dataSource: ${key}`, () => {
          expect(result[key].monthlyWordsPerConversation).toEqual(expectedData.monthlyWordsPerConversation);
          expect(result[key].dailyWords).toEqual(expectedData.dailyWords);
          expect(result[key].slidingWindowMeanDailyWords).toEqual(expectedData.slidingWindowMeanDailyWords);
