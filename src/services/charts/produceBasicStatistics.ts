...
const produceBasicStatistics = (
  ...
  const activeMonths = new Set(
    [...wordCounts, ...secondCounts].map((point) => `${point.year}-${point.month}`)
  ).size;
  const activeYears = new Set([...wordCounts, ...secondCounts].map((point) => point.year)).size;

  const sentPerActiveMonth =
    activeMonths > 0 ? Math.round(messageCounts.allMessages.sent / activeMonths) : 0;
  const receivedPerActiveMonth =
    activeMonths > 0 ? Math.round(messageCounts.allMessages.received / activeMonths) : 0;
